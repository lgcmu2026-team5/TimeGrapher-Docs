...
| EXP-B1 | R-B1, R-B4 | QAS-2 | High | 틱/톡 0.1ms 수준 검출 정밀도가 가능한가? |
| EXP-B2 | R-B2, R-D1 | QAS-2 | High | SNR 저하 시 검출률/오차 기준을 만족하는가? |
| EXP-F6 | R-F6 | QAS-4 | High | Pi 의존도를 줄인 검증체계를 만들 수 있는가? |
...
- Live 입력 또는 동일 Playback 파일 세트
- 지연/드롭 로깅 코드
- 작업 공수: 1.5 person-days

### 실험 설명
1. 48k/96k/192k를 동일 입력 조건에서 각각 10분 실행한다.
2. capture->processing, processing->display, total latency를 기록한다.
3. block drop, missed beat, CPU/RAM을 수집한다.
4. SAP 시나리오 기준으로 측정 결과를 판단한다.
	- Source: 마이크 입력 스트림
	- Stimulus: 연속 오디오 입력 도착
	- Artifact: 입력-분석-표시 파이프라인
	- Environment: RPi5 Live/Playback
	- Response: 실시간 처리 및 화면 갱신
	- Response Measure: total latency p99 <= 500 ms, block drop=0(목표)
...
## 실험 2: EXP-B1 (R-B1, R-B4)

### 결과 및 권장 사항
TO-DO: 틱/톡 검출 알고리즘 유지/개선/교체 결론을 기록한다.

### 목적
0.1 ms급 정밀도 검출 가능성을 ground truth 기반으로 검증한다.
핵심 질문은 다음과 같다.
- Q1. onset/peak 검출 오차가 목표 범위에 들어오는가?
- Q2. 오차가 rate/beat error/amplitude에 미치는 영향은 허용 가능한가?

### 상태
계획됨

### 예상 산출물
- 검출 시점 오차 분포(p50/p95/p99)
- 미검출/오검출 통계
- 파생지표 오차 전파 분석표

### 필요한 자원
- 합성신호 생성기(Sim)
- 정답 라벨(ground truth timestamp)
- Playback 재현 세트
- 작업 공수: 2.0 person-days

### 실험 설명
1. SNR, BPM, 진폭 조건을 스윕한 합성신호 세트를 생성한다.
2. 알고리즘 검출 결과와 정답 라벨 차이를 계산한다.
3. 미검출/오검출/시점오차를 집계한다.
4. 파생지표 오차 전파를 분석한다.
5. SAP 시나리오 기준으로 판단한다.
	- Source: 합성신호 생성기/Playback
	- Stimulus: 알려진 이벤트 시점 입력
	- Artifact: beat detection + 계산 모듈
	- Environment: Sim/Playback 반복 환경
	- Response: 이벤트 검출 및 파생지표 계산
	- Response Measure: 시점오차 p95 <= 0.1 ms(목표), 미검출 <= 5%
...
## 실험 3: EXP-B2 (R-B2, R-D1)

### 결과 및 권장 사항
TO-DO: 신호품질 임계값과 "signal weak" 게이팅 기준을 기록한다.

### 목적
잡음/약신호 환경에서 검출률과 정확도를 확인하고 graceful degradation 정책을 확정한다.
핵심 질문은 다음과 같다.
- Q1. SNR >= 14 dB에서 detection >= 95%를 달성하는가?
- Q2. 임계 미만에서 잘못된 수치 대신 signal weak를 안정적으로 표출하는가?
...
### 필요한 자원
- 노이즈 주입 가능한 Sim/Playback 벤치
- AGC on/off 제어 가능한 환경
- 성능 집계 스크립트
- 작업 공수: 2.0 person-days

### 실험 설명
1. SNR 단계(예: 8/10/12/14/16/20 dB)별 테스트 세트를 구성한다.
2. AGC on/off, 결합 상태(정상/불량)를 조합해 측정한다.
3. 검출률, rate 오차, 오표시율을 계산한다.
4. 임계값 후보를 비교해 신호품질 게이팅 기준을 선택한다.
5. SAP 시나리오 기준으로 평가한다.
	- Source: 잡음 섞인 입력/약신호 입력
	- Stimulus: SNR 저하, AGC 상태 변화
	- Artifact: 필터링 + 품질판정 + 표시 로직
	- Environment: Sim/Playback 노이즈 주입
	- Response: 허용구간은 측정, 임계 미만은 signal weak
	- Response Measure: SNR >= 14 dB에서 detection >= 95%, rate error <= +/-3 s/d
...
### 결과 및 권장 사항
TO-DO: Pi 의존도를 낮춘 표준 검증 전략(필수/비필수)을 기록한다.

### 목적
Pi 단일 장비 제약에서도 핵심 리스크를 일정 내 검증할 수 있는 테스트 전략을 수립한다.
핵심 질문은 다음과 같다.
- Q1. 핵심 시나리오를 Sim/Playback으로 얼마나 대체 검증 가능한가?
- Q2. Pi 실장비 필수 검증 항목의 최소 집합은 무엇인가?
...
### 실험 설명
1. 리스크별 검증 항목을 Pi 필수/비필수로 분류한다.
2. 비필수 항목은 Sim/Playback 회귀로 전환한다.
3. Pi 필수 항목은 성능/입출력/터치 중심 최소 집합으로 고정한다.
4. SAP 시나리오 기준으로 전략 적합성을 검토한다.
	- Source: 일정 및 장비 제약
	- Stimulus: Pi 접근 제한
	- Artifact: 테스트 아키텍처
	- Environment: 병행 개발 환경
	- Response: 검증 가능성 유지
	- Response Measure: 핵심 시나리오 70% 이상 Pi 없이 재현 검증
...
- TinyML on/off 성능 비교표(지연, 프레임타임, 오표시율)
...
2. 경량 모델 1~2종을 온디바이스에 탑재해 동일 입력으로 반복 측정한다.
...
5. SAP 시나리오 기준으로 평가한다.
	...
	- Response Measure: total latency p99 기준 유지, UI freeze 0회, 오표시율 감소
...
4. SAP 시나리오 기준으로 합격 여부를 판단한다.
	- Source: 장시간 연속 입력
	- Stimulus: 24h 지속 실행
	- Artifact: 버퍼/집계/표시 루프
	- Environment: Pi 장시간 실행
	- Response: 안정 동작 유지
	- Response Measure: 크래시 0회, RSS 증가율 안정, p99 latency 악화 없음
...
- UI 기준안: 최소 글자 높이(mm), 권장 글자 높이(mm), 최소 터치 타깃(mm)

### 필요한 자원
- Raspberry Pi 5 + 800x480 터치 디스플레이
...
- 큐/락 사용 구간의 대기시간 확인
3. 실시간 최적화 설계 패턴 후보를 제안하고, 현재 구조 적합도를 평가한다.
...
## 통합 일정
- Week 1: EXP-F2S, EXP-F6, EXP-A1, EXP-B1
- Week 2: EXP-B2, EXP-F4
- Week 3: EXP-A4, EXP-E1E3 및 미해결 항목 재실험
