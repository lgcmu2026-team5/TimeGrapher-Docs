...
| FR-12-12 | Desired | [EN] The system documentation shall note that low-amplitude signal components may become less visible in F1 mode.<br>[KO] 시스템 문서는 F1 모드에서 저진폭 신호 성분의 가시성이 저하될 수 있음을 명시해야 한다. | [EN] Project Plan Draft p.24 notes that moving-average smoothing in F1 can reduce the visibility of low-amplitude components.<br>[KO] Project Plan Draft p.24는 F1의 이동 평균 평활화가 저진폭 성분의 가시성을 저하시킬 수 있음을 언급한다. |
