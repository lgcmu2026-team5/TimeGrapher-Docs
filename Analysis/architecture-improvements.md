...
- **근거**: LCOM **96** / CBO **48** / RFC **75** / NIM 74 / NIV **44** / WMC **75**, DIT 1. MainWindow는 1541줄·1295코드·64유닛(전체 코드 ≈21.5%).
...
- **근거**: MainWindow가 mCtx/mCfg를 직접 생성·소유하고, UI→DSP 직접 의존과 높은 절차 복잡도가 함께 나타남.
...
- **Before→After 지표**: UI→DSP 직접 참조 edge 제거, 레이어 간 의존 위반 0건.
...
- **근거**: Windows WASAPI와 Linux ALSA 코드가 분리된 공통 포트 없이 유지되며, `ConfigureSoundCard`가 `WindowsSetSoundParameters` / `LinuxSetSoundParameters`를 직접 호출.
...
- **근거**: `find_c_onset` CC 19·중첩 5, `ProcessSamples` CC 21·중첩 6도 동일 징후.
...
- **근거**: `SharedAudio.h`의 `QMutex Mutex`, MainWindow의 워커 생성/종료 흐름.
