...
**[발표자]**
> "여섯 가지 품질 속성이 이 시스템을 이끌며, 이 순서로 우선순위가 정해집니다: **정확도(Accuracy)**가 최우선, 그 다음은 **성능/지연**, **신뢰성**, **일관성**, **수정 용이성**, **사용성**. 이것들은 일반적인 품질 단어가 아닙니다 — 측정 가능한 시나리오로 변환했습니다.
>
...
**[발표자]**
> "정확도를 우선시하면서 다른 곳에서 비용이 발생했고, 그 트레이드오프는 의도적으로 이루어졌습니다:
>
> **정확도 vs 지연**: BPH와 레이트를 보고하기 전 더 긴 워밍업은 첫 번째 측정값이 늦어진다는 뜻입니다 — 초기에 잘못된 수치를 보여주지 않기 위해 이 지연을 수용했습니다.
>
> **정확도 vs 성능**: 높은 샘플레이트는 더 정밀한 타임스탬프 해상도를 주지만 박자당 CPU 비용이 높아집니다. 저희는 192 kHz까지 지원하고 Pi에서도 예산 내에 있음을 검증했습니다.
>
> **워커 파이프-앤-필터 vs 핫패스 지연**: 입력, 분석, 렌더링, 기록을 워커 경계에서 분리하지만 — 검출기와 메트릭스는 의도적으로 하나의 동기 핫패스로 유지합니다. 매 단계마다 내부 큐잉을 하면 박자 주기 예산을 소모하게 됩니다.
>
> **핵심 트레이드오프 — 측정 vs 시각화**: 시스템이 마감 기한을 초과하면 *시각화 먼저* 저하됩니다 — 최신 우선 렌더링이 중간 프레임을 건너뛰지만 — **측정값은 절대 드롭하거나 보간하지 않습니다**. 수치를 보호하고 그림을 희생합니다."
...
**[발표자]**
> "저희는 AI를 세 가지 방식으로 활용했습니다.
>
> 첫 번째, **제품 기능으로서**: TimeGrapher.Inference 모듈은 Pi 온디바이스에서 실행되는 ONNX 기반 신호 품질 분류기입니다 — 네 가지 클래스, 박자당 여덟 가지 특징. 아키텍처 제약: 모델은 후보를 분류하고 거부합니다; 이벤트를 생성하거나, 재타이밍하거나, BPH 동기화를 건드릴 수 없습니다.
>
> 두 번째, **개발 협력자로서**: Qt-to-.NET 포팅, 디버깅, 933개 테스트, 문서, 아키텍처 검토 전반에 걸쳐 Claude Code와 Codex를 사용했습니다.
>
> 세 번째, **엔지니어링 자동화로서**: AI가 CI/CD 파이프라인 설계와 개선을 도왔습니다 — 빌드, 테스트, Verify 실행, 릴리스 패키징. 그 파이프라인은 제품이 아니지만, 저희가 변경하는 동안 제품을 보호합니다."
...
**[발표자]**
> "저희는 생명주기 전반에 AI를 적용했습니다: Qt-to-.NET 포팅, 테스트 생성, CI/CD 워크플로 자동화, 그리고 저희 포트를 원래 C++ 코드와 줄 단위로 비교한 AI 주도 **성능 회귀 감사** — 여섯 가지 회귀를 찾아 각각 문서화된 커밋으로 수정했습니다.
>
> 구체적인 예: 파이프-앤-필터 아키텍처 결정. UI 스레드가 무거운 스펙트로그램 계산 중에 멈추고 있었습니다. Claude에 병목을 설명하고 Bass, Clements & Kazman의 SAP에서 관련 패턴을 제안해달라고 했습니다. 생산자-소비자 + 최신 우선 스케줄러를 제안했습니다. 책의 품질 속성 분석에 대조해 검증하고 구현했으며, EXP-03이 UI 스레드가 이제 완전히 분리되었음을 확인합니다.
>
> 두 번째 예: ViewModel 순도 테스트. 'ViewModel에 Avalonia 없음' 제약을 기계적으로 강제하는 방법을 물었습니다. Claude가 리플렉션 기반 테스트를 제안했습니다. ViewModelPurityTests로 구현했고 — 모든 CI 푸시마다 실행됩니다.
>
...
> "솔직하게, 강점, 한계, 위험에 대해:
>
> **강점**: 소규모 팀이 대형 실시간 앱을 포팅하고, 온디바이스 분류기를 추가하고, 추적 가능한 히스토리로 빌드/테스트/릴리스 워크플로를 자동화할 수 있게 해줬습니다.
>
> **한계**: AI는 과장합니다. 초기에 저희 MVVM과 파이프-앤-필터를 완전히 적용된 것으로 표시했습니다. 코드에 대조해 검증하고 그 주장을 낮춰야 했습니다. 분류기에도 같이 적용됩니다: 실행 중인 앱에서 모델이 실제로 하는 것만 주장합니다.
>
