...
**[발표자]**
> "품질 속성 간에 트레이드오프가 발생했고, 그 선택은 모두 의도적이었습니다.
>
> **QAS-1 정확도 vs QAS-2 지연**: BPH와 레이트를 보고하기 전 더 긴 워밍업은 첫 번째 측정값이 늦어진다는 뜻입니다 — 초기에 잘못된 수치를 보여주지 않기 위해 이 지연을 수용했습니다. 또한 높은 샘플레이트는 정밀한 타임스탬프 해상도를 주지만 박자당 CPU 비용이 높아집니다 — 저희는 192 kHz까지 지원하고 Pi에서도 예산 내에 있음을 검증했습니다.
>
> **QAS-2 성능 → 파이프-앤-필터 선택**: 오디오 처리와 렌더링을 워커 경계에서 분리했습니다. 렌더링은 탭이 선택될 때만 발생하며, 13개 탭을 동시에 처리하지 않습니다. 단, 검출기와 메트릭스는 의도적으로 하나의 **동기 핫패스**로 유지합니다 — 매 단계마다 내부 큐잉을 하면 박자 주기 예산을 소모하기 때문입니다. 이 구조는 QAS-5 수정 용이성도 동시에 지원합니다: 새 탭을 추가해도 분석 파이프라인을 건드리지 않습니다.
>
> **QAS-6 사용성 → 중앙화**: 폰트 크기(2.9 mm)와 터치 영역(9 mm)은 Pi 터치스크린에서 실험으로 확정하고, App.axaml에 중앙화하여 유지보수 중 정책이 우발적으로 변경되지 않도록 강제합니다.
>
