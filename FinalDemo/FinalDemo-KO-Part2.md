...
>
> **아키텍처 레벨:** 화면의 레이어 다이어그램처럼, Core는 UI와 OS로부터 완전히 분리되어 있습니다. QAS-1이 목표를 정의합니다 — 깨끗한 입력에서 ≥1,000박자에 걸쳐 known reference 대비 ±1.0 s/d 이내. **Core가 무의존성**이기 때문에 **Verify 모듈**이 UI나 플랫폼 간섭 없이 Core를 완전한 격리 상태로 직접 실행해 정확도를 CI 매 변경마다 헤드리스로 검증할 수 있습니다.
>
> **구현 레벨:** 그 목표의 실제 달성은 Core.Detection이 담당합니다. 핵심 메커니즘은 **서브샘플 보간** — A 이벤트는 선형, C 이벤트는 포물선 — 으로, 192 kHz에서 정수 샘플 해상도보다 훨씬 정밀한 이벤트 타이밍을 산출합니다. 이 정밀도를 실제 환경에서 유지하기 위한 세 가지 추가 메커니즘이 있습니다. 첫째, **적응형 노이즈 바닥** — 주변 환경 소음 수준을 계속 감시해 기준값을 자동으로 조정합니다. 방이 조용하면 민감하게, 시끄러우면 둔하게 조절해 항상 시계 소리만 잡아냅니다. 둘째, **PLL 유도 게이팅** — 박자 리듬을 파악한 후에는 다음 박자가 언제쯤 와야 하는지를 예측합니다. 예상 시간 밖에 들어온 신호는 노이즈로 판단하고 무시합니다. 셋째, **레짐 가드** — 충격이나 노이즈로 박자가 갑자기 달라 보여도 즉시 반응하지 않습니다. 같은 변화가 3번 연속 나올 때만 측정값을 갱신합니다 — 단발 충격 하나가 측정 락을 깨지 못합니다.
>
