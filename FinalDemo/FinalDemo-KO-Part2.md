...
**[지시문]** 레이어 다이어그램 → Core.Detection 정확도 메커니즘 슬라이드.

**[슬라이드 비주얼 — 레이어 구조: Core 무의존성]**

![레이어 다이어그램](../Milestone/assets/LAYER.png)

**[슬라이드 비주얼 — Core.Detection 정확도 메커니즘]**

![Core.Detection — 정확도 메커니즘](assets/core-detection-tactics.svg)

**[발표자]**
> "정확도는 저희의 최우선 품질속성입니다. 목표는 QAS-1이 정의합니다 — 깨끗한 입력에서 ≥1,000박자에 걸쳐 known reference 대비 ±1.0 s/d 이내. 아키텍처는 이 목표를 검증 가능하게 만들었고, 실제 달성은 Core.Detection 구현이 담당합니다.
>
> **아키텍처 레벨:** 화면의 레이어 다이어그램처럼, Core는 UI와 OS로부터 완전히 분리되어 있습니다. **Core가 무의존성**이기 때문에 **Verify 모듈**이 UI나 플랫폼 간섭 없이 Core를 완전한 격리 상태로 직접 실행해 정확도를 CI 매 변경마다 헤드리스로 검증할 수 있습니다.
>
> **구현 레벨:** QAS 목표의 실제 달성은 Core.Detection이 담당합니다. 핵심 메커니즘은 **서브샘플 보간** — A 이벤트는 선형, C 이벤트는 포물선 — 으로, 192 kHz에서 정수 샘플 해상도보다 훨씬 정밀한 이벤트 타이밍을 산출합니다. 이 정밀도를 실제 환경에서 유지하기 위한 세 가지 추가 메커니즘이 있습니다. 첫째, **적응형 노이즈 바닥** — 주변 환경 소음 수준을 계속 감시해 기준값을 자동으로 조정합니다. 방이 조용하면 민감하게, 시끄러우면 둔하게 조절해 항상 시계 소리만 잡아냅니다. 둘째, **PLL 유도 게이팅** — 박자 리듬을 파악한 후에는 다음 박자가 언제쯤 와야 하는지를 예측하고 그 타이밍에 락(lock)합니다. 예상 시간 밖에 들어온 신호는 노이즈로 판단하고 무시합니다. 셋째, **레짐 가드** — 충격이나 노이즈로 박자가 갑자기 달라 보여도 즉시 반응하지 않습니다. 같은 변화가 3번 연속 나올 때만 측정값을 갱신합니다 — 단발 충격 하나가 이 락을 깨지 못합니다."
...
| 트레이드오프 | 우리의 선택 | 구현 |
|---|---|---|
| QAS-1 정확도 vs QAS-2 지연 | 정확도 우선 | 192 kHz + 긴 워밍업 수용 — Pi에서 예산 내임을 실험으로 사전 확인 |
| QAS-5 수정용이성 vs QAS-1·2 정확도·지연 | 핫패스는 분리 안 함 | 입력·분석·렌더링은 워커 경계에서 분리, 검출기+메트릭스는 단일 동기 패스 유지 |
| QAS-1 정확도 vs QAS-2 시각 응답성 | 측정값 보호 | 마감 초과 시 시각화 먼저 저하 (최신 우선 렌더링) — 측정값은 절대 드롭 안 함 |
| QAS-3 신뢰성 vs QAS-1 정확도 | 정확도 우선 | PLL 유도 게이팅 + 레짐 가드로 노이즈 환경에서도 타이밍 락 유지 |

**[발표자]**
> "품질 속성들 사이에는 트레이드오프가 있었고, 모든 선택은 의식적이었습니다.
>
...
**[발표자]**
> "**달성한 것:** 6가지 QAS 목표에 대해 모두 달성했습니다. 실측 수치는 다음 슬라이드에서 말씀드리겠습니다.
>
> **남은 한계:**
...
>
> 탭별: Filter Scope가 36.46 ms로 가장 느립니다. Escapement가 15.09 ms로 가장 빠릅니다. 13개 탭 모두 예산 내. 핵심 메커니즘: **최신 우선 스케줄러**가 새 프레임이 오면 처리하지 못한 이전 프레임을 버리기 때문에, 지연이 누적되지 않습니다."
...
>
> 2. **자동화된 검증** — Verify 콘솔이 CI의 모든 변경마다 검출된 BPH와 이벤트 수준의 정밀도/재현율을 실측 픽스처와 대조합니다. weak signal, noise, impulse storm, gain step 같은 adverse 시나리오가 거기서 게이팅됩니다.
>
> 3. **테스트 스위트** — Core, App, Platform 레이어 전체에서 **933개 테스트** 통과.
>
> 종합하면: 라이브 측정값이 Weishi Timegrapher와 일치하고, 검출기가 알려진 신호에 대해 자동으로 확인되며, 전체가 회귀 방지됩니다."
...
>
> 활용 영역은 다섯 가지입니다. **베이스 코드 변환** — Qt/C++ DSP 로직·이벤트 검출·오디오 버퍼를 C# 관용구(Span\<T\>, Channel, IDisposable)로 변환. **코드 구현** — 렌더러, 테스트 픽스처, 버퍼 풀 등 반복적인 구현 작업을 AI와 함께 작성하고 사람이 검토. **CI/CD 파이프라인** 설계와 자동화. **933개 테스트** 생성. **문서 번역** — 아키텍처 문서와 발표 스크립트의 한국어·영어 버전을 AI로 초안 작성 후 DocRules.md 기준으로 검토·수정."
...
**[발표자]**
> "모든 활용 영역에서 공통 원칙은 하나입니다: **AI가 초안을 작성하고, 사람이 실행 가능한 증거로 검증합니다.**
>
> **① 베이스 코드 변환.** 기존 Qt/C++ 구현을 C#으로 옮기는 작업이 가장 큰 초기 리스크였습니다. AI에게 C++ 파일을 컨텍스트로 주고 .NET 8 관용구(Span\<T\>, ArrayPool, Channel)로 변환을 요청했습니다. 단순 문법 변환이 아니라 포인터 산술 → Span, RAII → IDisposable, Qt 시그널 → C# 이벤트로 **관용구까지 바꾸는 변환**이었습니다. Verify 픽스처와 테스트 통과로 정확성을 확인했습니다.
>
> **② 코드 구현.** 렌더러 클래스, 버퍼 풀, 테스트 픽스처처럼 패턴이 반복되는 구현을 AI와 함께 작성했습니다. UI 스레드가 스펙트로그램 계산 중 멈추던 문제를 Claude에 설명하고 SAP 패턴을 물었더니 생산자-소비자 + 최신 우선 스케줄러를 제안했고, 책의 품질 속성 분석으로 검증한 뒤 구현했습니다. EXP-03이 UI 스레드 분리를 확인합니다.
>
> **③ CI/CD 파이프라인.** 빌드·테스트·릴리스 워크플로 설계를 AI에게 요청했습니다. Core 무의존성 경계 테스트와 tag-triggered 크로스 플랫폼 릴리스 구조를 제안받아 적용했습니다. 이후 파이프라인이 매 커밋마다 아키텍처 경계를 자동으로 검증합니다.
>
> **④ 테스트 생성.** 933개 테스트 중 상당수를 AI가 생성했습니다. 검출 임계값 근처의 엣지케이스 입력 생성, ViewModel에 Avalonia 타입이 들어오면 CI에서 잡는 ViewModelPurityTests 설계를 AI가 제안했고 리플렉션 기반으로 구현했습니다.
>
> **⑤ 문서 번역.** 아키텍처 문서와 발표 스크립트의 한국어·영어 버전을 AI로 초안 작성 후 DocRules.md 기준으로 검토·수정했습니다. 양쪽 버전이 동기화된 상태를 유지하면서도 각 언어에서 자연스럽게 읽히도록 사람이 최종 판단했습니다."
