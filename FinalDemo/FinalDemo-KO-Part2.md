...
>
> **구현 레벨:** 그 목표의 실제 달성은 Core.Detection이 담당합니다. 핵심 메커니즘은 **서브샘플 보간** — A 이벤트는 선형, C 이벤트는 포물선 — 으로, 192 kHz에서 정수 샘플 해상도보다 훨씬 정밀한 이벤트 타이밍을 산출합니다. 이 정밀도를 실제 환경에서 유지하기 위한 세 가지 추가 메커니즘이 있습니다: 침묵 샘플의 75번째 백분위수를 기준으로 노이즈 바닥을 자동 추적하는 **적응형 노이즈 바닥**, 락(lock) 후 예측된 박자 창 밖의 신호를 거부하는 **PLL 유도 게이팅**, 그리고 3회 연속 조건이 충족될 때만 상태를 리셋하는 **레짐 가드** — 단발 충격 하나가 측정 락을 깨지 못합니다.
>
