...
| QAS-3 신뢰성 vs QAS-1 정확도 | 정확도 우선, 사용자에게 알림 | PLL 유도 게이팅 + 레짐 가드로 타이밍 락 유지. 임계값 이하 신호에서는 측정을 강행하는 대신 '신호 약함' 알람을 표시 |
...
>
> **QAS-3(신뢰성) vs QAS-1(정확도)**: 노이즈 환경에서 검출률(QAS-3)을 높이려면 임계값을 낮춰야 하지만, 그러면 가짜 박자가 들어와 정확도(QAS-1)가 떨어집니다. 저희의 선택은 정확도를 보호하는 것이었습니다. 대신 신호가 임계값 이하로 떨어지면 측정을 강행하는 대신 사용자에게 '신호 약함' 알람을 표시합니다 — 틀린 수치보다 알람이 낫습니다. PLL 게이팅과 레짐 가드는 임계값 이상 신호에서 노이즈를 걸러내는 구현 전술입니다. TinyML 분류기도 이 트레이드오프를 다루지만, 이벤트를 생성하거나 재타이밍할 수 없도록 **아키텍처로 제한**했습니다 — 모델이 틀려도 타이밍 락을 깰 수 없습니다."
