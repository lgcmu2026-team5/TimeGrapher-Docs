...
이 뷰는 어떤 레이어가 어떤 하위 레이어를 사용할 수 있는지 보여준다. 구현 세부사항이 아니라, 나머지 뷰 전반의 모든 모듈 의존성을 지배하는 *허용된* 의존성을 정의한다. 여기서 정한 방향은 모든 다른 뷰가 지켜야 하는 아키텍처의 근간 제약이다.
...
- **Layer 3 – Portable Core**: `TimeGrapher.Core`(analysis, detection, metrics – 외부 의존성 없음).
