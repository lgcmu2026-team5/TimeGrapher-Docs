...
Analysis worker 내부의 DSP/metrics path는 별도 pipe/queue/thread로 쪼개지지 않는다. 같은 analysis thread에서 block 단위로 순차 실행되는 synchronous staged chain이다. 이 구조의 architecture view는 [Worker Pipeline View](../5-Architecture-Views/5-7-Worker-Pipeline-View.md)에 둔다. 이 ADR은 worker-level flow에는 Pipe-and-Filter를 적용하고, worker 내부 hot path에는 적용하지 않는 결정을 기록한다.
